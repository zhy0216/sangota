...
开发时可以在游戏内选择 `src/devScenes/scenes/` 下的可复现战斗状态：

```bash
bun run dev:scene
```

点击场景时会同时解锁浏览器音频，因此战斗音乐和第一张牌的音效都能正常播放。
仍可用 `bun run dev:scene scene1` 只显示指定场景的进入页。

`scene1.ts` 展示了完整格式：可指定遭遇、敌我体力与状态、敌方意图、气、手牌、
抽牌堆、弃牌堆、消耗堆、宝物和丹药。测试场景以自定义局运行，且不会读取、
覆盖或清除正常游戏存档；修改场景文件后刷新浏览器即可重置战斗。
