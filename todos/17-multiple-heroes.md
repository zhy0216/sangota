...
## 阶段四实测 · 卡池还差三分之二，两个新武将从没被量过

**牌池大小**（`poolFor(heroId, rarity)`，实测 common/uncommon/rare）：

| 武将 | common | uncommon | rare | 合计 |
|---|---|---|---|---|
| 关羽 | 10 | 8 | 3 | **21** |
| 赵云 | 3 | 3 | 3 | **9** |
| 诸葛亮 | 3 | 3 | 3 | **9** |

上面「实现步骤」第 3 条写的是「`cardPool` 列 20+ 张新牌」。差距不是抽象的：
坊市一次上 4 张本武将牌，赵云一进坊市就看到自己 9 张里的 4 张；
「秘录 · 得稀世之技三张」要从 3 张稀有池里发 3 张，也就是必然发全。

**胜率实测**（`npm run sim` 的「三将逐场」表，250 场/行，greedy）：
...
带外行数（同一套 band）：关羽 3/16、赵云 10/16、诸葛亮 13/16。
诸葛亮打三幕精英只掉 6-19% 血；赵云打天命 7% 胜。

**不要先调这两列的数值。** 因果在上游：池子只有 9 张，抽卡近乎确定性，
现在调出来的任何数字，等池子补到 20 张就作废。顺序是先补牌池，再调平衡。
