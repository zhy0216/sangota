...
## 阶段四实测 · 二三幕的池子偏薄（内容缺口，不是 bug）

阶段四修掉了终章的种子缺陷（`generateFinalAct` 曾经返回字面量 `'final'`，
导致整幕所有房间流与跑团种子无关；见 `tests/acts.test.ts`
「gives two seeds two different 终章」）。下面两条是量出来的**内容**缺口，
留在这里而不是改数值，因为它们要靠加内容解决。

**1. 普通战池太薄，一条路线内就会重复。**

| 幕 | weak / strong / elite / boss |
|---|---|
| 第一幕 | 3 / 6 / 3 / 3 |
| 第二幕 | **3 / 3** / 2 / 2 |
| 第三幕 | **2 / 4** / 2 / 2 |

沿实际可达节点随机走 50 条路线的实测：第一幕平均 8.6 场 / 8.4 种不同；
第二幕 8.5 场 / **6.1 种**；第三幕 8.5 场 / **7.0 种**。
`pickEncounter` 的去重只在池未吃空前有效（`fresh.length > 0 ? fresh : pool`），
第二幕第 6 场之后开始重复。二三幕各再要 2-3 个普通遭遇。

**2. 12 个奇遇撑不满四幕。**

`clearActProgress` 不清 `seenEvents`（这是对的，跨幕不该重复），但 `EVENTS`
只有 12 条。实测遍历每幕地图上全部 event 节点：第一幕 12 个节点见到 11 种，
